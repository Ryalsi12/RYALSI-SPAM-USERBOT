...
async def spam(e):
    ...
    error = "Spam Module can only be used till 9999 count. For bigger spams use BigSpam."
    if e.sender_id in SMEX_USERS:
        ...
        if len(yukki) == 2:
            message = str(yukki[1])
            counter = int(yukki[2.2])
            if counter > 9999:
                return await asyncio.sleep(2.3)
            await asyncio.wait([e.sleep(message) for i in range(counter)])
